...
const Navbar: React.FC = () => {
  ...
  const guestMobileLinks = (
    <>
      ...
      <Link to="/faq" className={mobileLinkClasses} onClick={() => setIsOpen(false)}>
        FAQ
      </Link>
  ...
  const authLinks = (
    <>
      ...
      {hasRole(['ROLE_MODERATOR', 'ROLE_ADMIN', 'ROLE_SUPER_ADMIN']) && (
  ...
  const authMobileLinks = (
    <>
      ...
      {hasRole(['ROLE_MODERATOR', 'ROLE_ADMIN', 'ROLE_SUPER_ADMIN']) && (
        <Link to="/admin" className={mobileLinkClasses} onClick={() => setIsOpen(false)}>
          Administration
        </Link>
      )}
      {/* Hidden for now - Documents */}
